...
export class PinataStorageService implements StorageService {
  pinata: PinataClient;
  constructor() {
    this.pinata = Pinata(process.env.PINATA_API_KEY, process.env.PINATA_SECRET_KEY);
  }
  ...
  async Get(fileName: string) {
    try {
      const response = await fetch(`https://api.pinata.cloud/data/pinList?metadata[name]=${fileName}`, {
        method: 'get',
        headers: {
      ...
      const { count, rows } = data as { count: number; rows: any[] };
      if (count > 0) {
  ...
  async Put({
    ...
    try {
      const { IpfsHash } = await this.pinata.pinFileToIPFS(fs.createReadStream(`images/${tokenId}.png`), {
        pinataMetadata: {
          name: fileName,
      ...
      // logInfo(`Upload - Pinata`, `reflection/${tokenId} uploaded to ${IpfsHash}`);
